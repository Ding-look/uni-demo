<template>
	<div class="shop-header" v-if="shopInfo.id">
		<nav class="shop-nav" :style="shopNavStyle"></nav>
		...
		<div class="shop-header-discounts" v-if="shopInfo.supports" @click="toggleSupportShow">
			<div class="discounts-left">
				<div class="activity" :class="supportFirstClass">
		...
		<transition name="fade">
			<div class="shop-brief-modal" :style="shopShowStyle">
		...
		<transition name="fade">
			<div class="activity-sheet" v-show="supportShow">
				<div class="activity-sheet-content">
					...
					<ul class="list">
						<li class="activity-item" v-for="(support, index) in shopInfo.supports" :key="index" :class="supportClasses[shopInfo.supports[index].type]">
							<span class="content-tag">
...
export default {
  data() {
    return {
      supportClasses: ["activity-green", "activity-red", "activity-orange"], // 优惠活动颜色class数组
      shopShow: false, // 控制商家信息面板显示/隐藏
      supportShow: false // 控制优惠活动列表显示/隐藏
    };
  },
  ...
  computed: {
	...
	// 商家背景图
    shopNavStyle() {
      return `background-image: url(${this.shopInfo.bgImg})`;
    },

	// 优惠活动标题颜色class(默认显示第一条)
	supportFirstClass () {
		const { supportClasses, shopInfo } = this;
		return shopInfo.supports && supportClasses[shopInfo.supports[0].type];
	},
	
	// 商家信息面板显示/隐藏
	shopShowStyle () {
		return this.shopShow || 'display:none'
	}

  },

  methods: {
    ...mapActions('shop', ["getShopInfo"]),
